    ...
    for epoch in range(opt.pretrain_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if 'cutmix' in opt.pt_aug:
                from augmentations import add_noise
                x_categ_corr, x_cont_corr = add_noise(x_categ,x_cont, noise_params = pt_aug_dict)
                _ , x_categ_enc_2, x_cont_enc_2 = embed_data_dict(x_categ_corr, x_cont_corr, cat_mask, con_mask,model,key,vision_dset)
            else:
                _ , x_categ_enc_2, x_cont_enc_2 = embed_data_dict(x_categ, x_cont, cat_mask, con_mask,model,key,vision_dset)
            _ , x_categ_enc, x_cont_enc = embed_data_dict(x_categ, x_cont, cat_mask, con_mask,model,key,vision_dset)
